...
// Next.js App Router, 'use client'. Без framer-motion. TailwindCSS.
// ВНИМАНИЕ: существующие тексты НЕ изменялись; добавлены "МИНОКА" и "Осложнения", а также
// выравнивание/порядок фильтров и размещение бейджей строго под заголовком.
...
const CARDIOLOGY_NOSOLOGIES: Nosology[] = [
  ...
  // Поражения клапанов и врожденные пороки
...
export default function ACSPage() {
  ...
  // ====== ДАННЫЕ ГАЙДА (СОХРАНЕНО И РАСШИРЕНО, НИЧЕГО НЕ УДАЛЯЛОСЬ) ======
  const escGuideline = {
    ...
    // ДИАГНОСТИКА (как было) + МИНОКА (добавлено)
    diagnosis: {
      initialAssessment: [
        {
      ...
      // ==== ДОБАВЛЕНО: МИНОКА (диагностический блок + леч. тактика) ====
      minoca: {
        title: 'МИНОКА (Инфаркт миокарда без обструктивного поражения коронарных артерий)',
        notes: [
          'Рабочий диагноз при ↑тропонина с клиникой ИМ и отсутствием стенозов ≥50% по КАГ; обязательна этиологическая верификация (спазм, диссекция, тромб/эмболия, микрососудистая дисфункция, миокардит, ТС и др.).',
          'Обязательны: повторная оценка ЭКГ/эхоКГ, КАГ/КТ-КАГ, по показаниям — МРТ сердца с поздним усилением, провокационные тесты на спазм, лабораторный поиск причин эмболии/тромбофилии.',
        ],
        therapy: [
          {
            drug: 'Антиагреганты (аспирин ± P2Y12)',
            dose: 'Аспирин 150–300 мг нагрузка, затем 75–100 мг/сут; P2Y12 по показаниям',
            note: 'Назначение зависит от предполагаемого механизма (тромбоз бляшки vs иные механизмы).',
            class: 'IIa' as RecommendationClass,
            level: 'C' as EvidenceLevel,
            evidenceText: 'IIa/C — «следует рассмотреть» при вероятном тромботическом генезе.',
          },
          {
            drug: 'Статины высокой интенсивности',
            dose: 'Аторвастатин 40–80 мг/сут или Розувастатин 20–40 мг/сут',
            note: 'Цель ЛПНП <1.4 ммоль/л и/или снижение ≥50% от исходного.',
            class: 'I' as RecommendationClass,
            level: 'B' as EvidenceLevel,
            evidenceText: 'I/B — при атеросклеротической природе и/или высоком риске.',
          },
          {
            drug: 'ИАПФ/БРА',
            dose: 'Рамиприл 2.5→10 мг/сут; Периндоприл 2→8 мг/сут; Валсартан 80→320 мг/сут',
            note: 'Снижение риска ремоделирования; особенно при дисфункции ЛЖ/СН/АД.',
            class: 'IIa' as RecommendationClass,
            level: 'B' as EvidenceLevel,
          },
          {
            drug: 'Бета-блокаторы',
            dose: 'Метопролол 25–50 мг 2 р/сут или Бисопролол 2.5–10 мг/сут',
            note: 'Полезны при тахикардии/ишемии и дисфункции ЛЖ; избегать при вазоспазме.',
            class: 'IIb' as RecommendationClass,
            level: 'C' as EvidenceLevel,
          },
          {
            drug: 'Антиспастическая терапия',
            dose: 'Дилтиазем 120–360 мг/сут или Амлодипин 5–10 мг/сут; нитраты по требованию',
            note: 'При документированном коронарном спазме — терапия выбора.',
            class: 'I' as RecommendationClass,
            level: 'B' as EvidenceLevel,
          },
          {
            drug: 'Антикоагуляция',
            dose: 'По индивидуальным показаниям (эмболия/тромбофилия), подбор по массе/КК',
            note: 'При эмболическом генезе; исключить источники эмболии (ФП, клапаны, тромб ЛП).',
            class: 'IIa' as RecommendationClass,
            level: 'C' as EvidenceLevel,
          },
        ],
    ...
    // ЛЕЧЕНИЕ (как было — без сокращений)
    treatment: {
      generalMeasures: [
        {
    ...
    // ===== ДОБАВЛЕНО: ОСЛОЖНЕНИЯ (после Лечения, до Профилактики) =====
    complications: [
      {
        title: 'No-reflow / Slow-flow после ЧКВ',
        dx: ['TIMI {'<'} 3, низкий myocardial blush grade', 'Персистирующая ишемия/боль/ЭКГ изменения после стентирования'],
        algo: [
          'Исключить механические причины (диссекция/тромб/спазм) — повторная ангиография',
          'Интракоронарно: аденозин 60–120 мкг болюс, при необходимости повтор; нитропрепараты (нитроглицерин 100–200 мкг)',
          'Эптифибатид (ингибитор IIb/IIIa): 180 мкг/кг в/в болюс ×2 с интервалом 10 мин, затем 2 мкг/кг/мин 12–18 ч',
          'Мягкая дилатация/аспирация тромба по показаниям',
        ],
        class: 'IIa' as RecommendationClass,
        level: 'B' as EvidenceLevel,
      },
      {
        ...
        dx: ['САД {'<'} 90 мм рт.ст. {'>'}30 мин или необходимость вазопрессоров', 'Признаки гипоперфузии (олигурия, лактоацидоз, холодные кожные покровы)'],
        algo: [
          'Немедленное ЧКВ «culprit-only», при необходимости — механическая поддержка кровообращения',
          'Вазопрессор: норэпинефрин титровать до целевого MAP',
          'Инотроп: добутамин при низком сердечном выбросе',
          'ИВЛ/кислородотерапия по показаниям',
        ],
        class: 'I' as RecommendationClass,
        level: 'A' as EvidenceLevel,
      },
      {
        ...
        dx: ['Одышка, хрипы, гипоксемия, рентген-признаки застоя'],
        algo: [
          'Кислород/НИВЛ; при гипоксемии — ИВЛ',
          'Диуретик: фуросемид 20–40 мг в/в болюс, повтор/титрация',
          'Вазодилататоры: нитроглицерин в/в 5–10 мкг/мин с титрацией',
          'Лечение причины (реперфузия, контроль АД и ЧСС)',
        ],
        class: 'I' as RecommendationClass,
        level: 'B' as EvidenceLevel,
      },
      {
        title: 'Жизнеопасные аритмии (устойчивая ЖТ/ФЖ)',
        dx: ['Гемодинамическая нестабильность или остановка кровообращения'],
        algo: [
          'Немедленная дефибрилляция/кардиоверсия по алгоритму ACLS',
          'Амиодарон 300 мг в/в болюс, затем 150 мг при рецидиве; инфузия 900 мг/24 ч',
          'Коррекция электролитов (K+/Mg2+), устранение ишемии',
        ],
        class: 'I' as RecommendationClass,
        level: 'A' as EvidenceLevel,
      },
      {
        title: 'Контраст-индуцированное повреждение почек (CI-AKI)',
        dx: ['Рост креатинина {'>'}26.5 мкмоль/л в 48 ч или {'>'}1.5× от исходного'],
        algo: [
          'Гидратация: 0.9% NaCl 1–1.5 мл/кг/ч за 3–12 ч до и 6–12 ч после (если возможно)',
          'Минимизировать объём контраста; использовать изо- или низкоосмолярные агенты',
          'Избегать нефротоксичных ЛС (НПВП и т.п.), рассмотреть временную отмену метформина',
        ],
        class: 'I' as RecommendationClass,
        level: 'B' as EvidenceLevel,
      },
    ...
    // ===== ДОБАВЛЕНО: СОПУТСТВУЮЩИЕ СОСТОЯНИЯ (изменяющие терапию) =====
    comorbidity: [
      {
        area: 'Сахарный диабет и метформин при контрасте',
        items: [
          {
            drug: 'Метформин',
            dose: 'Обычно 500–1000 мг 2 р/сут (фон)',
            note: 'При КК {'<'} 30 мл/мин — противопоказан. При CI-AKI/риске её развития и введении контраста: временная отмена на 48 ч и до подтверждения стабильной функции почек.',
            class: 'I' as RecommendationClass,
            level: 'B' as EvidenceLevel,
          },
        ],
      ...
      {
        area: 'ФП/показания к ОАК после ЧКВ',
        items: [
          {
            drug: 'ОАК + антиагреганты',
            dose: 'Минимизировать тройную терапию; предпочтение ОАК + клопидогрел, как можно короче тройная (1–7 дней) у HBR.',
            note: 'Выбор ОАК и длительность — по рискам ишемии/кровотечений.',
            class: 'IIa' as RecommendationClass,
            level: 'B' as EvidenceLevel,
          },
        ],
  ...
  return (
    <main className="min-h-screen bg-[#fcfcee] py-8">
      <div className="max-w-[1800px] mx-auto px-4">
        {/* Заголовок и фильтры — ПОЛНОСТЬЮ ПЕРЕСТРОЕНО ПОД ЗАДАЧУ */}
        <section className="border-b border-gray-200 mb-8">
          <div className="max-w-[1800px] mx-auto px-4 pt-4 pb-4">
            <div className="grid grid-cols-1 lg:grid-cols-3 items-center gap-4">
              {/* Нозология (desktop: слева; mobile: второй) */}
              <div className="order-2 lg:order-1">
                <div className="flex flex-col gap-1 w-full">
                  ...
                  <select
                    ...
                    className="
                      rounded-full border border-[#d3cec4] bg-white px-4
                      h-12 lg:h-10 min-h-[48px] lg:min-h-0
                      text-base text-[#3b342d] text-center
                      shadow-sm focus:outline-none focus:border-[#015d52]
                      w-full
                    "
                  >
              ...
              {/* Заголовок (desktop: по центру; mobile: третий) */}
              ...
              {/* Специальность (desktop: справа; mobile: первый) */}
              <div className="order-1 lg:order-3">
                <div className="flex flex-col items-stretch lg:items-end gap-1 w-full">
            ...
            {/* Ссылки и предупреждение (без изменений) */}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-full border px-3 py-2 text-sm text-left font-medium transition
                        ${active ? 'bg[#013c37] text-white border-[#013c37]'.replace('[#','[#') : 'bg-white text-[#1f2933] border-[#d3cec4]'}
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Калькуляторы риска — desktop, под табами (оригинальные тексты сохранены) */}
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8">
              ...
              {selectedTab === 'diagnosis' && (
                <div className="space-y-12">
                  ...
                  {/* Калькуляторы риска (как были внутри Диагностики) */}
                  <section>
                  ...
                  {/* ДОБАВЛЕНО: МИНОКА (визуализация в Диагностике) */}
                  <section>
                    <h3 className="text-2xl font-semibold text-gray-900 mb-6">МИНОКА: диагностика и тактика</h3>
                    <div className="bg-amber-50 rounded-xl p-6 border border-amber-200">
                      <div className="space-y-3 mb-4">
                        {escGuideline.diagnosis.minoca.notes.map((t, i) => (
                      ...
                      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {escGuideline.diagnosis.minoca.therapy.map((t, i) => (
                          <div key={i} className="bg-white rounded-xl p-4 border border-amber-100">
                            ...
                            <p className="text-sm text-gray-700 mb-1"><strong>Доза/режим:</strong> {t.dose}</p>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      {escGuideline.treatment.generalMeasures.map((measure, idx) => (
                        <div key={idx} className="bg-blue-50 rounded-xl p-6 border border-blue-200">
                          ...
                          {Array.isArray((measure as any).contraindications) && (
                            <div className="mt-3">
                              <p className="font-medium text-sm mb-1">Противопоказания:</p>
                              <ul className="text-sm text-gray-700 space-y-1">
                                {(measure as any).contraindications.map((contra: string, i: number) => (<li key={i}>• {contra}</li>))}
                              </ul>
                            </div>
                          )}
                        </div>
                  ...
                  <section>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                          {(escGuideline.treatment.antiplateletTherapy[1] && escGuideline.treatment.antiplateletTherapy[1].options || []).map((drug: any, idx: number) => (
                      ...
                      {escGuideline.treatment.anticoagulation.map((ac: any, idx: number) => {
                        ...
                        return (
                          <div key={idx} className="bg-white rounded-xl p-6 border border-gray-200">
                            <div className={`flex ${isLongTitle ? 'flex-col' : 'flex-col'} gap-4 mb-3`}>
                      ...
                      {escGuideline.treatment.adjunctiveTherapy.map((therapy: any, idx: number) => {
                        ...
                        return (
                          <div key={idx} className="bg-purple-50 rounded-xl p-6 border border-purple-200">
                            <div className={`flex ${isLongTitle ? 'flex-col' : 'flex-col'} gap-4 mb-4`}>
              ...
              {selectedTab === 'complications' && (
                <div className="space-y-8">
                  <h2 className="text-3xl font-bold text-gray-900 mb-6">Осложнения при ОКС и после ЧКВ</h2>
                  <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {escGuideline.complications.map((c: any, idx: number) => (
                      <div key={idx} className="rounded-xl p-6 border bg-white"
                           style={{borderColor:'#e5e7eb', boxShadow:'0 0 0 1px rgba(0,0,0,0.02)'}}>
                        <div className="flex flex-col gap-3 mb-3">
                          <h4 className="text-lg font-semibold text-gray-900">{c.title}</h4>
                          <RecommendationBadge rec={{ class: c.class, level: c.level }} />
                        </div>
                        <p className="text-sm text-gray-700 mb-2 font-medium">Диагностика:</p>
                        <ul className="text-sm text-gray-700 space-y-1 mb-3">
                          {c.dx.map((t: string, i: number) => (<li key={i}>• {t}</li>))}
                        </ul>
                        <p className="text-sm text-gray-700 mb-2 font-medium">Алгоритм лечения:</p>
                        <ul className="text-sm text-gray-700 space-y-1">
                          {c.algo.map((t: string, i: number) => (<li key={i}>• {t}</li>))}
                        </ul>
                      </div>
                    ))}
                  </div>
                  ...
                  <h3 className="text-2xl font-semibold text-gray-900 mt-10 mb-4">Сопутствующие состояния, изменяющие тактику</h3>
                  <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {escGuideline.comorbidity.map((blk: any, i: number) => (
                      <div key={i} className="rounded-xl p-6 border bg-white"
                           style={{borderColor:'#e5e7eb', boxShadow:'0 0 0 1px rgba(0,0,0,0.02)'}}>
                        <h4 className="text-lg font-semibold text-gray-900 mb-2">{blk.area}</h4>
                        {blk.items.map((t: any, j: number) => (
                          <div key={j} className="bg-slate-50 rounded-lg p-4 border border-slate-200 mb-3">
                            <div className="flex flex-col gap-2 mb-1">
                              <h5 className="font-semibold text-gray-900">{t.drug}</h5>
                              <RecommendationBadge rec={{ class: t.class, level: t.level, evidenceText: t.evidenceText }} />
                            </div>
                            <p className="text-sm text-gray-700"><strong>Доза/режим:</strong> {t.dose}</p>
                            {t.note && <p className="text-xs text-gray-600 mt-1">{t.note}</p>}
                          </div>
                        ))}
                      </div>
