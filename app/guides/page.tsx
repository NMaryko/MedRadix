// app/guides/acs/page.tsx — ОКС (финал «код-окс»: мобильные фильтры, MINOCA, осложнения, сопутствующие; без сокращений)

'use client';

import React, { useState } from 'react';
import { ExternalLink, Clock, CheckCircle, AlertTriangle, FolderPlus, ChevronDown } from 'lucide-react';
...
const escGuideline = {
  ...
  version: 'ESC 2023–2024', // источник общий — используем как ориентир
  sources: {
  ...
  // ДИАГНОСТИКА (как было — без сокращений) + добавлен блок MINOCA в конце
  diagnosis: {
    initialAssessment: [
      {
    ...
    // ДОБАВЛЕНО: MINOCA — без сокращений, с конкретикой
    minoca: {
      title: 'МИНОКА (Myocardial Infarction with Non-Obstructive Coronary Arteries)',
      definition: [
        'Клинические критерии ИМ (симптомы ишемии, тропонин с динамикой, ишемические изменения ЭКГ/Эхо/МРТ)',
        'Отсутствие обструктивных стенозов (все эпикардиальные сегменты <50% по КАГ/КТ-КАГ)',
        'Отсутствие альтернативы, полностью объясняющей повреждение миокарда (миокардит, ТЭЛА, ТСМ и т.д.)',
      ],
      workup: [
        'Исключить миокардит/такоцубо: КМРТ с LGE/Т2',
        'Провести внутриисследовательские методики при КАГ при необходимости: OCT/IVUS для исключения эрозии/разрыва бляшки/SCAD',
        'Оценить вазоспазм: ацетилхолиновая/эргометриновая проба (в специализированных центрах)',
        'Исключить эмболию/тромбофилию: Эхо с пузырьковым тестом, мониторинг аритмий, коагулограмма',
      ],
      treatment: [
        {
          topic: 'Антитромбоцитарная терапия',
          bullets: [
            'Если подтверждён тромбоз/эрозия бляшки: ДАТТ 12 мес: Аспирин 75–100 мг/сут + Тикагрелор 90 мг 2 раза/сут или Клопидогрел 75 мг/сут (Класс I, Уровень B)',
            'Если вазоспастический механизм: монотерапия аспирином 75–100 мг/сут (Класс IIa, Уровень C)',
          ],
        },
        {
          topic: 'Антиангинальная / вазоспастическая терапия',
          bullets: [
            'БКК: Дилтиазем 120–360 мг/сут или Амлодипин 5–10 мг/сут (Класс I, Уровень B)',
            'Долгие нитраты: Изосорбида мононитрат 20–60 мг/сут (Класс IIa, Уровень B)',
          ],
        },
        {
          topic: 'Модификация риска/кардиопротекция',
          bullets: [
            'Статины высокой интенсивности: Аторвастатин 40–80 мг/сут или Розувастатин 20–40 мг/сут (Класс I, Уровень A)',
            'ИАПФ/БРА при сниженной ФВ ЛЖ/СД/АГ: Рамиприл 2.5–10 мг/сут, Валсартан 80–320 мг/сут (Класс I, Уровень A)',
            'Бета-блокаторы при тахикардии/симптомах: Бисопролол 2.5–10 мг/сут (Класс IIa, Уровень B)',
          ],
        },
      ],
    },
  ...
  // ЛЕЧЕНИЕ (как было — сохранил; добавлены только безопасные TS-обёртки)
  treatment: {
    generalMeasures: [
      {
...
export default function ACSPage() {
  ...
  // Безопасный доступ к P2Y12 опциям
  const p2y12Block = (escGuideline.treatment.antiplateletTherapy as any[]).find((x) => x && 'options' in x);
  const p2y12Options: any[] = p2y12Block?.options ?? [];
        ...
        <section className="border-b border-gray-200 mb-8">
          <div className="max-w-[1800px] mx-auto px-4 pt-4 pb-4">
            <div className="flex flex-col lg:flex-row items-center gap-4 lg:gap-0">
              {/* Специальность — на мобиле первая */}
              <div className="flex-1 flex justify-start order-1 w-full lg:w-auto lg:order-1 lg:justify-start">
                <div className="flex flex-col items-start lg:items-start gap-1 w-full lg:w-auto">
                  <span className="text-[11px] uppercase tracking-[0.18em] text-[#9c978f] hidden lg:block">Специальность</span>
                  <select
                    value={selectedSpecialty}
                    onChange={(e) => setSelectedSpecialty(e.target.value)}
                    className="rounded-full border border-[#d3cec4] bg-white text-center px-4 h-12 min-h-[48px] text-base text-[#3b342d] shadow-sm focus:outline-none focus:border-[#015d52] w-full lg:w-[220px] lg:h-10 lg:min-h-0 lg:text-sm lg:px-3"
                  >
              ...
              {/* Нозология — на мобиле вторая */}
              <div className="flex-1 flex justify-end order-2 w-full lg:w-auto lg:order-3 lg:justify-end">
                <div className="flex flex-col gap-1 w-full lg:w-auto">
                  ...
                  <select
                    ...
                    className="rounded-full border border-[#d3cec4] bg-white text-center px-4 h-12 min-h-[48px] text-base text-[#3b342d] shadow-sm focus:outline-none focus:border-[#015d52] w-full lg:w-[260px] lg:h-10 lg:min-h-0 lg:text-sm lg:px-3"
                  >
                    {Object.entries(groupedCardiologyNosologies).map(([groupName, items]) => (
                      <optgroup key={groupName} label={groupName}>
                        {items.map((nos) => (
                          <option key={nos.id} value={nos.label}>
                            {nos.label}
                          </option>
                        ))}
                      </optgroup>
              ...
              {/* Заголовок — на мобиле идёт ниже фильтров (order-3), на десктопе по центру */}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8">
                ...
                <div className="space-y-12">
                    ...
                    {isIntroOpen && (
                      <div className="border border-emerald-200 border-t-0 rounded-b-xl p-4 space-y-4">
                        <p className="text-gray-700">
                          ОКС — острое клиническое проявление ишемической болезни сердца, вызванное несоответствием между коронарным кровотоком и потребностью миокарда в
                          кислороде. Основные механизмы: разрыв/эрозия бляшки с тромбозом, спазм, микроциркуляторная дисфункция.
                        </p>
                        <p className="text-gray-700">
                          Классификация: STEMI (персистирующий подъём ST/новая БЛНПГ) и NSTE-ACS (NSTEMI/нестабильная стенокардия). Стратегия определяется ЭКГ, тропонином и
                          шкалами риска (GRACE/HEART).
                        </p>
                  ...
                  <section>
                    ...
                    <div className="space-y-6">
                      {escGuideline.diagnosis.initialAssessment.map((step: any, index: number) => (
                  ...
                  <section>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-red-50 rounded-xl p-6 border border-red-200">
                        ...
                        {escGuideline.diagnosis.ecgCriteria.stemi.map((criteria: any, idx: number) => (
                          <div key={idx} className="mb-6 last:mb-0">
                            ...
                            <ul className="text-sm text-gray-700 space-y-1">
                              {criteria.details.map((detail: string, i: number) => (
                                <li key={i}>• {detail}</li>
                              ))}
                            </ul>
                            {criteria.examples && (
                              <div className="mt-2">
                                ...
                                <ul className="text-sm text-gray-600 space-y-1">
                                  {criteria.examples.map((example: string, i: number) => (
                                    <li key={i}>• {example}</li>
                                  ))}
                                </ul>
                              </div>
                      ...
                      <div className="bg-orange-50 rounded-xl p-6 border border-orange-200">
                        ...
                        {escGuideline.diagnosis.ecgCriteria.nstemi.map((criteria: any, idx: number) => (
                          <div key={idx} className="mb-6 last:mb-0">
                            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
                              <h5 className="font-semibold text-gray-900">{criteria.criteria}</h5>
                              <RecommendationBadge rec={{ class: criteria.class, level: criteria.level, evidenceText: criteria.evidenceText }} />
                  ...
                  <section>
                    ...
                    <div className="bg-purple-50 rounded-xl p-6 border border-purple-200">
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <ul className="text-sm text-gray-700 space-y-1">
                            {escGuideline.diagnosis.biomarkers.highSensitivityTroponin.interpretation.map((item: string, idx: number) => (
                              <li key={idx}>• {item}</li>
                            ))}
                          </ul>
                        </div>
                    ...
                    <div className="mt-6">
                      ...
                      <ul className="text-sm text-gray-700 space-y-1">
                        {escGuideline.diagnosis.biomarkers.additionalMarkers.map((marker: string, idx: number) => (
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-yellow-50 rounded-xl p-6 border border-yellow-200">
                        ...
                        <ul className="text-sm text-gray-700 space-y-1 mb-4">
                          {escGuideline.diagnosis.riskStratification.grace.parameters.map((param: string, idx: number) => (
                            <li key={idx}>• {param}</li>
                          ))}
                        </ul>
                        <p className="font-medium mb-2">Уровни риска:</p>
                        <ul className="text-sm text-gray-700 space-y-1">
                          {escGuideline.diagnosis.riskStratification.grace.scores.map((score: string, idx: number) => (
                            <li key={idx}>• {score}</li>
                          ))}
                        </ul>
                      </div>

                      {/* HEART (у вас названа как hematics) */}
                      <div className="bg-green-50 rounded-xl p-6 border border-green-200">
                        ...
                        <p className="font-medium mb-2">Параметры:</p>
                        <ul className="text-sm text-gray-700 space-y-1 mb-4">
                          {escGuideline.diagnosis.riskStratification.hematics.parameters.map((param: string, idx: number) => (
                            <li key={idx}>• {param}</li>
                          ))}
                        </ul>
                        <p className="font-medium mb-2">Уровни риска:</p>
                        <ul className="text-sm text-gray-700 space-y-1">
                          {escGuideline.diagnosis.riskStratification.hematics.scores.map((score: string, idx: number) => (
                            <li key={idx}>• {score}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {/* ДОБАВЛЕНО: MINOCA */}
                  <section>
                    <h3 className="text-2xl font-semibold text-gray-900 mb-6">МИНОКА (MINOCA): диагностика исключения и ведение</h3>
                    <div className="bg-amber-50 rounded-xl p-6 border border-amber-200 space-y-4">
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        <h4 className="text-lg font-semibold text-amber-800">Определение и диагностический алгоритм</h4>
                        <RecommendationBadge rec={{ class: 'IIa', level: 'B', evidenceText: 'Класс IIa — следует рассмотреть; Уровень B' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1">
                        {escGuideline.diagnosis.minoca.definition.map((d: string, i: number) => (
                          <li key={i}>• {d}</li>
                        ))}
                      </ul>
                      <p className="font-medium">Обследование:</p>
                      <ul className="text-sm text-gray-700 space-y-1">
                        {escGuideline.diagnosis.minoca.workup.map((d: string, i: number) => (
                          <li key={i}>• {d}</li>
                        ))}
                      </ul>

                      {/* Терапия MINOCA с дозами */}
                      <div className="grid md:grid-cols-2 gap-4">
                        {escGuideline.diagnosis.minoca.treatment.map((blk: any, idx: number) => (
                          <div key={idx} className="bg-white/70 border border-amber-200 rounded-lg p-4">
                            <div className="flex items-start justify-between gap-4 mb-2">
                              <h5 className="font-semibold text-gray-900">{blk.topic}</h5>
                              <RecommendationBadge rec={{ class: 'I', level: 'A', evidenceText: 'См. пункты с указанием класса/уровня в каждой строке' }} />
                            </div>
                            <ul className="text-sm text-gray-700 space-y-1">
                              {blk.bullets.map((b: string, j: number) => (
                                <li key={j}>• {b}</li>
                              ))}
                            </ul>
                          </div>
              ...
              {selectedTab === 'treatment' && (
                <div className="space-y-12">
                  ...
                  <section>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      {escGuideline.treatment.generalMeasures.map((measure: any, idx: number) => (
                        <div key={idx} className="bg-blue-50 rounded-xl p-6 border border-blue-200">
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                            ...
                            <RecommendationBadge
                              rec={{ class: measure.class ?? 'I', level: measure.level ?? 'A', evidenceText: 'evidenceText' in measure ? (measure.evidenceText as string) : undefined }}
                            />
                          </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                          <div>
                            ...
                            <p className="text-gray-600">
                              Нагрузка: {(escGuideline.treatment.antiplateletTherapy as any[])[0].loading} • Поддержка:{' '}
                              {(escGuideline.treatment.antiplateletTherapy as any[])[0].maintenance}
                            </p>
                          </div>
                          <RecommendationBadge
                            rec={{
                              class: ((escGuideline.treatment.antiplateletTherapy as any[])[0].class as RecommendationClass) ?? 'I',
                              level: ((escGuideline.treatment.antiplateletTherapy as any[])[0].level as EvidenceLevel) ?? 'A',
                              evidenceText: ((escGuideline.treatment.antiplateletTherapy as any[])[0].evidenceText as string) ?? undefined,
                            }}
                          />
                        </div>
                        <p className="text-gray-700">{(escGuideline.treatment.antiplateletTherapy as any[])[0].notes}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                          {p2y12Options.map((drug: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-xl p-6 border-2 border-blue-200 shadow-sm">
                              <div className="flex flex-wrap md:flex-nowrap md:items-start md:justify-between gap-4 mb-4">
                                ...
                                <RecommendationBadge rec={{ class: drug.class ?? 'I', level: drug.level ?? 'A', evidenceText: drug.evidenceText }} />
                              </div>
                        ...
                        return (
                          <div key={idx} className="bg-white rounded-xl p-6 border border-gray-200">
                            <div className={`flex ${isLongTitle ? 'flex-col' : 'flex-col md:flex-row md:items-start md:justify-between'} gap-4 mb-3`}>
                              ...
                              <RecommendationBadge
                                rec={{ class: ac.class ?? 'I', level: ac.level ?? 'A', evidenceText: 'evidenceText' in ac ? (ac.evidenceText as string) : undefined }}
                              />
                            </div>
                        ...
                        {escGuideline.treatment.reperfusion.stemi.map((method: any, idx: number) => (
                          <div key={idx} className="mb-6 last:mb-0">
                            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
                              ...
                              <RecommendationBadge rec={{ class: method.class ?? 'I', level: method.level ?? 'A', evidenceText: method.evidenceText }} />
                            </div>
                            ...
                            {method.drugs && (
                              <div className="mt-2">
                                ...
                                <ul className="text-sm text-gray-700 space-y-1">
                                  {method.drugs.map((d: string, i: number) => (
                                    <li key={i}>• {d}</li>
                                  ))}
                                </ul>
                              </div>
                        ...
                        {escGuideline.treatment.reperfusion.nstemi.map((strategy: any, idx: number) => (
                          <div key={idx} className="mb-6 last:mb-0">
                            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
                              ...
                              <RecommendationBadge rec={{ class: strategy.class ?? 'I', level: strategy.level ?? 'A', evidenceText: strategy.evidenceText }} />
                            </div>
                            <ul className="text-sm text-gray-700 space-y-1">
                  ...
                  <section>
                    ...
                    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                      {escGuideline.treatment.adjunctiveTherapy.map((therapy: any, idx: number) => {
                        const isLong = therapy.drug === 'Антагонисты минералокортикоидных рецепторов';
                        return (
                          <div key={idx} className="bg-purple-50 rounded-xl p-6 border border-purple-200">
                            <div className={`flex ${isLong ? 'flex-col' : 'flex-col md:flex-row md:items-start md:justify-between'} gap-4 mb-4`}>
                              <h4 className="text-lg font-semibold text-gray-900">{therapy.drug}</h4>
                              <RecommendationBadge
                                rec={{ class: therapy.class ?? 'I', level: therapy.level ?? 'A', evidenceText: 'evidenceText' in therapy ? (therapy.evidenceText as string) : undefined }}
                              />
                            </div>
                            <p className="text-gray-700 mb-2">
              ...
              {selectedTab === 'complications' && (
                <div className="space-y-12">
                  <h2 className="text-3xl font-bold text-gray-900 mb-6">Осложнения при ОКС: диагностика и ведение</h2>

                  <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {/* No-reflow */}
                    <div className="rounded-xl p-6 border bg-indigo-50 border-indigo-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">No-reflow / No-reperfusion при ЧКВ</h3>
                        <RecommendationBadge rec={{ class: 'IIa', level: 'B', evidenceText: 'Эскалация аденозина/верапамила/нитратов; ГП IIb/IIIa — по показаниям' }} />
                      </div>
                      <p className="text-sm text-gray-700 mb-2 font-medium">Диагностика:</p>
                      <ul className="text-sm text-gray-700 space-y-1 mb-3">
                        <li>• TIMI < 3, миокардиальный blush grade низкий, персистирующая ишемия после стентирования</li>
                      </ul>
                      <p className="text-sm text-gray-700 mb-2 font-medium">Алгоритм лечения:</p>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>• Аденозин 60–120 мкг интракоронарно болюсно; можно повторять (Класс IIa, Уровень B)</li>
                        <li>• Верапамил 100–200 мкг ИК болюсно (Класс IIa, Уровень B)</li>
                        <li>• Нитроглицерин 100–200 мкг ИК болюсно (Класс IIa, Уровень C)</li>
                        <li>• ГП IIb/IIIa (Эптифибатид 180 мкг/кг ×2 болюса с интервалом 10 мин, затем 2 мкг/кг/мин инфузия 12–18 ч) при тромботическом компоненте (Класс IIa, Уровень B)</li>
                        <li>• Механическая аспирация тромба — селективно (Класс IIb, Уровень C)</li>
                      </ul>
                    </div>

                    {/* Кардиогенный шок */}
                    <div className="rounded-xl p-6 border bg-rose-50 border-rose-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">Кардиогенный шок</h3>
                        <RecommendationBadge rec={{ class: 'I', level: 'A', evidenceText: 'Немедленное реваскуляризация + поддержка перфузии' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1 mb-3">
                        <li>• Немедленная реваскуляризация инфаркт-связанной артерии (ЧКВ) (Класс I, Уровень A)</li>
                        <li>• Норадреналин титрованно (начало 0.05–0.1 мкг/кг/мин) для поддержания MAP ≥65 мм рт. ст. (Класс I, Уровень B)</li>
                        <li>• Добутамин 2.5–20 мкг/кг/мин при низком сердечном выбросе (Класс IIa, Уровень B)</li>
                        <li>• Инвазивный мониторинг; рассмотреть МПК (Impella/VA-ECMO) при неэффективности фармакотерапии (Класс IIa, Уровень B)</li>
                      </ul>
                    </div>

                    {/* Острая СН/отёк лёгких */}
                    <div className="rounded-xl p-6 border bg-teal-50 border-teal-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">Острая СН / отёк лёгких</h3>
                        <RecommendationBadge rec={{ class: 'I', level: 'A', evidenceText: 'Оксигенация, нитраты, петлевые диуретики' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>• Кислород таргетно SatO₂ 92–96% (Класс I, Уровень C)</li>
                        <li>• Нитроглицерин в/в 10–20 мкг/мин с титрацией (Класс IIa, Уровень B)</li>
                        <li>• Фуросемид 20–40 мг в/в болюсно (Класс I, Уровень B)</li>
                        <li>• NIV (CPAP/BiPAP) при гипоксемии/дыхательной недостаточности (Класс IIa, Уровень B)</li>
                      </ul>
                    </div>

                    {/* Жизнеугрожающие аритмии */}
                    <div className="rounded-xl p-6 border bg-yellow-50 border-yellow-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">Жизнеугрожающие аритмии</h3>
                        <RecommendationBadge rec={{ class: 'I', level: 'A', evidenceText: 'АЛС алгоритм, ранняя дефибрилляция' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>• VF/pVT: дефибрилляция по АЛС (Класс I, Уровень A)</li>
                        <li>• Устойчивая ЖТ с пульсом: электрическая кардиоверсия (Класс I, Уровень B)</li>
                        <li>• Брадиаритмии с гипоперфузией: атропин 0.5 мг болюсно, повтор до 3 мг; временная стимуляция при неэффективности (Класс IIa, Уровень B)</li>
                        <li>• Коррекция электролитов: MgSO₄ 2 г в/в при torsades (Класс I, Уровень B)</li>
                      </ul>
                    </div>
                    ...
                    <div className="rounded-xl p-6 border bg-slate-50 border-slate-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">Механические осложнения ИМ</h3>
                        <RecommendationBadge rec={{ class: 'I', level: 'B', evidenceText: 'Неотложная хирургия / МПК' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>• Разрыв МЖП/сосочковой мышцы/свободной стенки: срочная консультация кардиохирурга (Класс I, Уровень B)</li>
                        <li>• Внутриаортальная баллонная контрпульсация — мост к хирургии (Класс IIa, Уровень B)</li>
                      </ul>
                    </div>

                    {/* Контраст-индуцированное повреждение почек (CIN) */}
                    <div className="rounded-xl p-6 border bg-lime-50 border-lime-200">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">Контраст-индуцированное повреждение почек</h3>
                        <RecommendationBadge rec={{ class: 'IIa', level: 'B', evidenceText: 'Гидратация, минимизация контраста, нефро-защита' }} />
                      </div>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>• Изотонический NaCl 1–1.5 мл/кг/ч за 3–12 ч до и 6–12 ч после (Класс I, Уровень A)</li>
                        <li>• Минимизировать объём контраста; низкоосмолярные/изоосмолярные агенты (Класс IIa, Уровень B)</li>
                        <li>• Избегать НПВП/нефротоксичных препаратов</li>
                        <li>• Метформин: при eGFR 30–60 — временно прекратить в день процедуры и возобновить через 48 ч при стабильном креатинине (Класс IIa, Уровень C)</li>
                      </ul>
                    </div>
                  </div>

                  {/* Сопутствующие патологии — меняющие терапию */}
                  <section>
                    <h3 className="text-2xl font-semibold text-gray-900 mb-4">Сопутствующие состояния: коррекция терапии</h3>
                    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                      {/* СД и метформин */}
                      <div className="rounded-xl p-6 border bg-cyan-50 border-cyan-200">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <h4 className="text-lg font-semibold text-gray-900">Сахарный диабет: метформин и контраст</h4>
                          <RecommendationBadge rec={{ class: 'IIa', level: 'C', evidenceText: 'Безопасность после контраста / eGFR' }} />
                        </div>
                        <ul className="text-sm text-gray-700 space-y-1">
                          <li>• eGFR 30–60 мл/мин/1.73м²: метформин приостановить в день КАГ/ЧКВ и возобновить через 48 ч при стабильном креатинине</li>
                          <li>• eGFR &lt;30: метформин противопоказан</li>
                          <li>• Контроль гликемии: инсулиновые схемы короткого действия при гипергликемии &gt;10–11 ммоль/л</li>
                        </ul>
                      </div>

                      {/* ХБП и антикоагулянты */}
                      <div className="rounded-xl p-6 border bg-fuchsia-50 border-fuchsia-200">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <h4 className="text-lg font-semibold text-gray-900">ХБП: дозировки антикоагулянтов</h4>
                          <RecommendationBadge rec={{ class: 'I', level: 'A', evidenceText: 'Коррекция доз НМГ; фондапаринукс противопоказан при КК &lt;30' }} />
                        </div>
                        <ul className="text-sm text-gray-700 space-y-1">
                          <li>• Эноксапарин: при КК &lt;30 мл/мин — 1 мг/кг п/к 1×/сут</li>
                          <li>• Фондапаринукс: избегать при КК &lt;30 мл/мин</li>
                          <li>• НФГ предпочтителен при тяжёлой ХБП (контроль АЧТВ)</li>
                        </ul>
                      </div>

                      {/* Фибрилляция предсердий и ДАТТ */}
                      <div className="rounded-xl p-6 border bg-violet-50 border-violet-200">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <h4 className="text-lg font-semibold text-gray-900">ФП + ЧКВ: тройная терапия</h4>
                          <RecommendationBadge rec={{ class: 'IIa', level: 'A', evidenceText: 'Сократить длительность ТАТ; DOAC предпочтителен' }} />
                        </div>
                        <ul className="text-sm text-gray-700 space-y-1">
                          <li>• Тройная терапия (АСК + клопидогрел + DOAC) как можно короче (≤1 нед), затем двойная (DOAC + клопидогрел) до 6–12 мес</li>
                          <li>• Избегать прасугрела/тикагрелора в ТАТ из-за кровотечений</li>
                        </ul>
                      </div>
                    </div>
                  </section>
                      ...
                      {escGuideline.secondaryPrevention.medications.map((med: any, idx: number) => (
                        <div key={idx} className="bg-white rounded-lg p-4 border border-blue-100">
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-2">
                            ...
                            <RecommendationBadge rec={{ class: med.class ?? 'I', level: med.level ?? 'A', evidenceText: med.evidenceText }} />
                          </div>
                          {med.duration && (
                            <p className="text-sm text-gray-700 mb-1">
                          ...
                          {med.deescalation && <ul className="text-sm text-gray-700 space-y-1 mt-2">{med.deescalation.map((it: string, i: number) => <li key={i}>• {it}</li>)}</ul>}
                          {med.escalation && <ul className="text-sm text-gray-700 space-y-1 mt-2">{med.escalation.map((it: string, i: number) => <li key={i}>• {it}</li>)}</ul>}
                        </div>
                      ))}
                    </div>
                  </section>
                      ...
                      {escGuideline.secondaryPrevention.lifestyle.map((item: any, idx: number) => (
                        <div key={idx} className="bg-white rounded-lg p-4 border border-green-100">
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-2">
                            ...
                            <RecommendationBadge rec={{ class: item.class ?? 'I', level: item.level ?? 'A', evidenceText: item.evidenceText }} />
                          </div>
                          <p className="text-sm text-gray-700 mb-1">
                      ...
                      {escGuideline.comparison.keyDifferences.map((diff: any, idx: number) => (
                        <div key={idx} className="bg-white rounded-lg p-4 border border-green-100 shadow-sm">
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-3">
                            ...
                            <RecommendationBadge rec={{ class: diff.class ?? 'I', level: diff.level ?? 'A', evidenceText: diff.evidenceText }} />
                          </div>
            ...
            <section className="border-t border-gray-200 mt-12 pt-8 text-center">
              <p className="text-sm text-gray-600 mb-6 max-w-4xl mx-auto">
                Данное руководство представляет собой обзор и интерпретацию клинических рекомендаций для медицинских специалистов. Информация носит исключительно образовательный
                характер и не заменяет профессиональное медицинское заключение. При принятии клинических решений всегда следуйте официальным руководствам и локальным протоколам
                вашего учреждения. Авторы не несут ответственности за использование представленной информации в клинической практике.
              </p>

              {/* Большой отступ перед e-mail */}
